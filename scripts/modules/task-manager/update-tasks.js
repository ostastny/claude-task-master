...
import {
  ...
  isSilentMode,
  getCurrentTag
  , flattenTasksWithSubtasks, findProjectRoot
} from '../utils.js'
...
async function updateTasks (
  ...
  if (isMCP) { logFn.info(`updateTasks called with context: session=${!!session}`) } else logFn('info', 'updateTasks called') // CLI log
  ...
  try {
    ...
    if (tasksToUpdate.length === 0) {
      if (isMCP) { logFn.info(`No tasks to update (ID >= ${fromId} and not 'done').`) } else { logFn('info', `No tasks to update (ID >= ${fromId} and not 'done').`) }
