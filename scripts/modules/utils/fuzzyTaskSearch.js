...
export class FuzzyTaskSearch {
  ...
  formatSearchSummary (searchResults, options = {}) {
    ...
    if (includeBreakdown && breakdown) {
      const parts = []
      if (breakdown.highRelevance.length > 0) { parts.push(`${breakdown.highRelevance.length} high relevance`) }
      if (breakdown.mediumRelevance.length > 0) { parts.push(`${breakdown.mediumRelevance.length} medium relevance`) }
      if (breakdown.lowRelevance.length > 0) { parts.push(`${breakdown.lowRelevance.length} low relevance`) }
      if (breakdown.categoryTasks.length > 0) { parts.push(`${breakdown.categoryTasks.length} category matches`) }
