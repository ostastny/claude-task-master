...
export class BDDReplacementSystem {
		...
		if (!this.prdParser) {
			...
			this.prdParser = {
				parseRequirements: (content) => [
					{ title: 'Mock Requirement', description: content }
				],
				mapToGherkin: (req) => ({
					feature: req.title,
					scenarios: [{ name: 'Mock Scenario' }]
				}),
				generateGherkinFeature: (data) =>
			...
			this.featureEngine = {
				getFilenameSanitizer: () => ({
					sanitizeFilename: (name) =>
						`${name.toLowerCase().replace(/[^a-z0-9]/g, '-')}.feature`
				})
			};
		}
	}
	...
	async generateBDDFromRequirements(requirements, outputDir) {
		...
		try {
			...
			throw new Error(
				`Failed to generate BDD from requirements: ${error.message}`
			);
		}
	}
	...
	async validateFeatureFilesOnly(outputDir) {
		try {
			...
			throw new Error(
				`Failed to validate feature files only: ${error.message}`
			);
		}
	}
