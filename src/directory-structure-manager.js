...
export class DirectoryStructureManager {
	...
	generateAndPlaceFeature(featureData) {
		...
		const content =
			this.featureGenerationEngine.generateFeatureFile(featureData);
...
class PathHandler {
	...
	isValidPath(inputPath) {
		...
		if (process.platform === 'win32' && !this._isValidWindowsPath(inputPath))
			return false;
	...
	_isValidWindowsPath(inputPath) {
		return (
			this._hasValidWindowsReservedNames(inputPath) &&
	...
	_hasValidWindowsReservedNames(inputPath) {
		...
		return !pathParts.some((part) => {
			...
			return PathHandler.WINDOWS_RESERVED_NAMES.includes(
	...
	_hasValidWindowsCharacters(inputPath) {
		return !PathHandler.WINDOWS_INVALID_CHARS.some((char) =>
			inputPath.includes(char)
		);
	}
	...
	isAbsolute(inputPath) {
		return (
			this._isWindowsAbsolute(inputPath) ||
...
class CompatibilityTester {
	...
	testFilename(filename) {
		...
		return {
			isValid:
				pathHandler.isValidPath(filename) &&
				filename.length <= PathHandler.MAX_PATH_LENGTH
		};
	}
