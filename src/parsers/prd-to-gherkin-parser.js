...
export class PrdToGherkinParser {
  constructor() {
    this.parser = new Parser(new AstBuilder(IdGenerator.uuid()), new GherkinClassicTokenMatcher());
  }
  ...
  parseRequirements(prdContent) {
    ...
    for (const line of lines) {
      ...
      else if (this.isAcceptanceCriteriaItem(line) && currentSection === 'acceptanceCriteria') {
        if (currentRequirement) {
          currentRequirement.acceptanceCriteria.push(line.replace(/^[-*]\s*/, ''));
        }
      }
  ...
  isFeatureTitle(line) {
    return /^#+\s+/.test(line) || // Markdown headers
           ...
           (/^[A-Z]/.test(line) && line.length < 100 && !line.includes('As a') && !line.includes('I want'));
  }
  ...
  mapToGherkin(requirement) {
    const feature = requirement.title || this.extractTitleFromUserStory(requirement.userStory);
  ...
  generateBasicSteps(requirement) {
    ...
    const userStoryMatch = requirement.userStory.match(/^As a (.+?),?\s*I want (?:to\s+)?(.+?)(?:\s+so that\s+(.+))?$/i);
  ...
  validateGherkinSyntax(gherkinContent) {
    try {
      ...
      const invalidSteps = lines.filter(line => {
        ...
        if (!trimmed || trimmed.startsWith('Feature:') || trimmed.startsWith('Scenario:') || 
            trimmed.startsWith('Given') || trimmed.startsWith('When') || trimmed.startsWith('Then') ||
            trimmed.startsWith('And') || trimmed.startsWith('But') || trimmed.startsWith('#')) {
          return false;
        }
        // Check for lines that look like steps but don't have proper keywords
        return trimmed.length > 0 && /^[A-Z]/.test(trimmed) && !trimmed.includes(':');
      });
      
      if (invalidSteps.length > 0) {
        return {
          isValid: false,
          errors: [`Invalid steps without proper Gherkin keywords: ${invalidSteps.join(', ')}`],
          ast: null
        };
      }
  ...
  parsePrdToGherkin(prdContent) {
    ...
    return {
      ...
      errors: features.filter(f => !f.isValid).map(f => ({ title: f.title, errors: f.errors }))
    };
  }
}
