...
jest.unstable_mockModule('../../../../../src/feature-file-generation-engine.js', () => mockFeatureEngine)
jest.unstable_mockModule('../../../../../src/parsers/prd-to-gherkin-parser.js', () => mockPRDParser)
...
describe('Task 109: Generate BDD Files (Replacement for Task File Generation)', () => {
    ...
    mockTaskData = {
      tasks: [
        ...
        },
        {
          ...
          status: 'pending',
          priority: 'medium',
          details: 'Users should be able to view and edit their profile information',
          testStrategy: 'Unit tests for profile functions, UI tests for profile forms',
          subtasks: []
        }
      ]
    }
  ...
  Scenario: Edit profile information
    ...
    mockFeatureEngine.generateFeatureFile.mockResolvedValue({
      success: true,
      filePath: '/test/project/.taskmaster/features/user-authentication.feature',
      content: mockFeatureData[0].content
    })

    mockPRDParser.mapToGherkin.mockReturnValue(mockTaskData.tasks)
    mockPRDParser.generateGherkinFeature.mockReturnValue(mockFeatureData[0].content)
  ...
  describe('Task File System Replacement', () => {
    describe('validateNoBDDTaskFiles', () => {
      it('should validate that no task files are generated', async () => {
        await expect(
          GenerateBDDFiles.validateNoBDDTaskFiles(path.join(mockProjectRoot, '.taskmaster/tasks'))
    ...
    describe('cleanupTaskFiles', () => {
      it('should remove existing task files when migrating to BDD', async () => {
        await expect(
          GenerateBDDFiles.cleanupTaskFiles(path.join(mockProjectRoot, '.taskmaster/tasks'))
  ...
  describe('Integration with Existing System', () => {
    ...
    describe('File System Integration', () => {
      ...
      it('should handle concurrent file operations', async () => {
        ...
        await expect(Promise.all(concurrentOperations)).rejects.toThrow('generateBDDFiles not implemented')
      })
    })
  })
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    describe('Edge Cases', () => {
      ...
      it('should handle very large task lists', async () => {
        const largeTasks = Array(1000).fill(0).map((_, i) => ({
          id: i + 1,
          title: `Task ${i + 1}`,
  ...
  describe('Performance and Quality', () => {
    describe('Performance Tests', () => {
      it('should maintain acceptable performance for large projects', async () => {
        const largeProjectTasks = Array(100).fill(0).map((_, i) => ({
          id: i + 1,
          title: `Feature ${i + 1}`,
          description: `Description for feature ${i + 1}`,
          subtasks: Array(10).fill(0).map((_, j) => ({
            id: j + 1,
            title: `Subtask ${j + 1}`,
