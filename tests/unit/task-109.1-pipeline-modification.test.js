...
describe('Task 109.1: Update Main Task Generation Pipeline', () => {
	...
	beforeEach(async () => {
		...
		parsePRDModule = await import(
			'../../scripts/modules/task-manager/parse-prd.js'
		);
		expandTaskModule = await import(
			'../../scripts/modules/task-manager/expand-task.js'
		);
		generateTaskFilesModule = await import(
			'../../scripts/modules/task-manager/generate-task-files.js'
		);
		parsePRDDirectModule = await import(
			'../../mcp-server/src/core/direct-functions/parse-prd.js'
		);
		expandTaskDirectModule = await import(
			'../../mcp-server/src/core/direct-functions/expand-task.js'
		);
		generateTaskFilesDirectModule = await import(
			'../../mcp-server/src/core/direct-functions/generate-task-files.js'
		);
	...
	describe('Parse-PRD Command Pipeline Modification', () => {
		test('should route parse-prd to BDD system instead of task generation', async () => {
			...
			const result = await parsePRDModule.parsePRD(
				'test-prd.txt',
				'output.json',
				5,
				{}
	...
	describe('Task Expansion Pipeline Modification', () => {
		test('should route task expansion to BDD scenario generation', async () => {
			...
			const result = await expandTaskModule.expandTask(
				'tasks.json',
				'1',
				3,
				{}
	...
	describe('Core File Generation Pipeline Modification', () => {
		test('should replace generateTaskFiles with BDD feature generation', async () => {
			...
			const result = await generateTaskFilesModule.generateTaskFiles(
				'tasks.json',
				'output'
			);
		...
		test('should route MCP generateTaskFilesDirect to BDD system', async () => {
			// Test that the MCP direct function now works with BDD system
			const result =
				await generateTaskFilesDirectModule.generateTaskFilesDirect({
					projectRoot: '/workspace',
					outputDir: 'output'
				});
			expect(result).toEqual({ success: true, data: { message: 'Success' } });
